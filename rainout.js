...
function openRainoutModal(gameId) {
  ...
  // ── AGENT: notify bus so ConflictDetector warning can appear in modal
  if (typeof AgentBus !== 'undefined') {
    setTimeout(() => AgentBus.publish('rainout:opened', { gameId }), 100);
  }
}
...
function renderRainoutModal(game) {
  ...
  modal.innerHTML = `
    <div style="background:white;padding:24px;border-radius:12px;max-width:420px;width:90%;box-shadow:0 4px 20px rgba(0,0,0,0.2)">
      ...
      <div id="agent-rainout-recs"
           style="display:none;margin-bottom:14px;padding:8px 10px;background:#fef9c3;
                  border:1px solid #fde68a;border-radius:7px;font-size:12px;color:#92400e"></div>
...
function confirmRainout() {
  ...
  saveData();
  // Surgical row update — avoid wiping score inputs currently on screen.
